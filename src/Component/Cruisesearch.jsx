...
const Cruisesearch = () => {
  ...
  const fetchShipAllList = async ()=>{
    try {
      let result = await axios.get(`${API_SHIP_BASE_URL}?app_id=${APP_ID}&token=${TOKEN}&limit=100`, {
        headers: {
  ...
  const fetchOperatorAllList = async ()=>{
    try {
      let result = await axios.get(`${API_OPERATOR_BASE_URL}?app_id=${APP_ID}&token=${TOKEN}&limit=100`, {
        headers: {
          'Accept': 'application/json;api_version=2'    
        }
      }); 
  ...
  const searchCruises = async () => {
    try {
      ...
      if(cruiseLineQuery){
        ...
        cruiseLineId = matchingOperator?.id || null;
      }
      let convertFormatCruiseDateString = '';
      if(selectedCruiseDateString){
        convertFormatCruiseDateString = selectedCruiseDateString ? format(parse(selectedCruiseDateString, "d MMMM yyyy", new Date()), "yyyy-MM-dd"): "";
      }
   ...
   // Only add cruise_type if it exists
      if (cruiseTypeQuery) {
        queryParams.append("cruise_type", cruiseTypeQuery);
      }
        ...
        <div className="row">
          <div className="col-lg-2">
            <div className="form_c">
              ...
              <Select options={cruiseTypes} value={selectedCruiseTypes} onChange={setSelectedCruiseTypes} placeholder="Select Cruise Type" />
            </div>
          </div>
          <div className="col-lg-2">
            <div className="form_c">
              <span><i className="ri-map-pin-fill"></i> LOCATION</span>
              <Select options={regions} value={selectedRegions} onChange={setSelectedRegions} placeholder="Select Region" />
            </div>
          </div>
          ...
          <div className="col-lg-2">
            <div className="form_c">
              ...
              <Select options={cruiseDate} value={selectedCruiseDate} onChange={setSelectedCruiseDate} placeholder="Select Date" />
            </div>
          </div>
          <div className="col-lg-2">
            <div className="form_c">
              <span><i className="ri-ship-line"></i> Cruise line</span>
              <Select options={cruiseLines} value={selectedCruiseLine} onChange={setSelectedCruiseLine} placeholder="Select Cruise Line" />
            </div>
          </div>
          <div className="col-lg-2">
            <div className="form_c">
              <span><i className="ri-ship-line"></i> Ship name</span>
              <Select options={ships} value={selectedShips} onChange={setSelectedShips} placeholder="Select Ship" />
            </div>
          </div>
          <div className="col-lg-2">
            <div className="form_c">
              <button onClick={searchCruises} className="search_btn">Search</button>
            </div>
          </div>
        </div>
      </div>
