...
import CruiseCarousel from "./carousel";
...
const Exploreluxury = () => {
  ...
  //       ...new Set(result.cruises.flatMap((cruise) => cruise?.general_categories)),
  //     ];
  ...
  const fetchShipdataFromDB = async () => {
    try {
      ...
      const uniqueCruiseTypes = [
        ...new Set(filterResult?.flatMap((cruise) => cruise?.general_type)),
      ];

      // Filter out "River"
      const filteredCruiseTypes = uniqueCruiseTypes.filter(
        (type) => type !== "River" && type.trim() !== ""
      );
      setCruiseTypes(
        filteredCruiseTypes.map((type) => ({ value: type, label: type }))
      );
              ...
              <div className="tab-content p-3" id="nav-tabContent">
                {cruiseTypes.map((type) => (
                  ...
                  >
                    ...
                    {/* <CruiseCarousel cruiseDataFromDB={cruiseDataFromDB} type={type}/> */}
                  </div>
                ))}
              </div>
            </>
          ) : !cruiseDataFromDB?.length && !cruiseTypes?.length && loading ? (
            <div className="d-flex justify-content-center align-items-center h-25">
              <div className="spinner-border text-secondary" role="status">
                <span className="sr-only"></span>
              </div>
            </div>
          ) : !cruiseDataFromDB?.length && !cruiseTypes?.length ? (
